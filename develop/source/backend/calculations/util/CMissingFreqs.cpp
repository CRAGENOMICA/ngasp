...
int CMissingFreqs::myround(float r){
  // xx check , why is this necessary?
    if (r-floorf(r)<0.5) return (int)floorf(r);
  return (int)ceilf(r);
}

float CMissingFreqs::lnfact(int n){
...
float CMissingFreqs::lnbinomial(int n, int k){
...
float CMissingFreqs::beta(int n,int i){
  int d1=n-i;
  if (d1==0) return 0;
  return 2.0*n*(a(n+1)-a(i))/((float)d1*((float)d1+1.0)) - 2.0/(float)d1;
}

float CMissingFreqs::ps(int k, int l, int n, float theta){
  int mkl=CComparison::min(k,l);
...
//
float CMissingFreqs::sigma(int i, int j,int n){
  int tmp;
...
//
float CMissingFreqs::pe(int k, int l, int n, float theta){
  float res=0.0;
  if (k+l>n) return res;
  if (k+l==n){
...
float CMissingFreqs::cs(int i, int j, int k, int l, int nx, int ny, int nxy,float theta){
  ...
  int lb = CComparison::max(i-nxy,l-j);
  int ub = CComparison::min3(i,nx-nxy,k-j);
  int kx;
  float sum=0.0;
  for(kx=lb;kx<=ub;kx++){
...
float CMissingFreqs::ce(int i, int j, int k, int l, int nx, int ny, int nxy,float theta){
  ...
  int lb = CComparison::max3(0,i-nxy,k-ny+j);
  int ub = CComparison::min(i,nx-nxy+j-l);
  int kx;
  float sum=0.0;
...
float CMissingFreqs::p(int i, int j, int nx, int ny, int nxy, float theta){
  int lb = 1;
  int ub = nx +ny -nxy -1 ;
  float sum = 0.0;
  float term;
...
float CMissingFreqs::cov_missing(int i, int j, int nx, int ny, int nxy, float theta) {
  return p(i,j,nx,ny,nxy,theta)- theta*theta/(i*j);
}

float CMissingFreqs::hn(int n){
...
void CMissingFreqs::print_conf_matrix(int *conf,int nconf, int n){
    int i,j;
    for (i=0;i<nconf;i++){
        for(j=0;j<n;j++){
...
float CMissingFreqs::watterson(int* a, int n){
  float an=hn(n-1);
...
float CMissingFreqs::corrected_watterson(int*a, int* r, int n){
  int j;
  int s=0;
...
void CMissingFreqs::store_configurations(int n,int s ,int* conf,int *row){
  /*
...
int CMissingFreqs::check_conf(int j, int n,int* conf){
...
float CMissingFreqs::hellman_prod(int noind, int* r,int* i){
...
float CMissingFreqs::hellman_sum(int *conf,int nconf, int noind, int *r){
  /*
...
float CMissingFreqs::fhat(int* a, float*c, int n,float* var){
...
void CMissingFreqs::_log(char *s){
  fprintf(stderr,"log:%s\n",s);
}

float CMissingFreqs::tajima_pi(int *a,float *c, int n){
  int k,l;
...
float CMissingFreqs::watterson_variance(float theta, int l, int* nx, int* ny,int* nxy, float* var_d, float* var_h){
  float asum=0.0;
  int x;
  for(x=0;x<l;x++){
    asum+=a(nx[x]);
