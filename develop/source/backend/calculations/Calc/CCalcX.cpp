...
CCalcX::CCalcX()
: ICalculation(KeyString::CALC_X,
               KeyString::CALC_X_BRIEF_DESC,
               KeyString::CALC_X_DESC,
  ...
  BEGIN_CALCULATION_INTERFACE_DEFINITION
    ...
    SET_INPUT_INFO(length,                                                      // Variable
                   ...
                   UNDEFINED_VALUE,                                             // Max. Value
                   OPTTYPE_mandatory)                                           // Required
    SET_INPUT_INFO(length_mask,                                                 // Variable
                   UNDEFINED_STRING,                                            // Group
                   UNDEFINED_STRING,                                            // Short Name
                   UNDEFINED_STRING,                                            // Long Name
                   UNDEFINED_STRING,                                            // Description
                   ...
                   UNDEFINED_VALUE,                                             // Max. Value
                   OPTTYPE_mandatory)                                           // Required
    SET_INPUT_INFO(sum_sam_mask,                                                // Variable
                   UNDEFINED_STRING,                                            // Group
                   UNDEFINED_STRING,                                            // Short Name
                   UNDEFINED_STRING,                                            // Long Name
                   UNDEFINED_STRING,                                            // Description
    ...
    SET_OUTPUT_INFO(length_al,                                                  // Variable
                   ...
                   UNDEFINED_VALUE,                                             // Max. Value
                   OPTTYPE_mandatory)                                           // Required
    SET_OUTPUT_INFO(flaghky,                                                    // Variable
                   UNDEFINED_STRING,                                            // Group
                   UNDEFINED_STRING,                                            // Short Name
                   UNDEFINED_STRING,                                            // Long Name
                   UNDEFINED_STRING,                                            // Description
...
void CCalcX::Prepare(void) {
  DM_GET_INPUTS
    ...
    DM_INPUT(length)
    DM_INPUT(length_mask)
    DM_INPUT(sum_sam_mask)
    DM_INPUT(nmhits)
    DM_INPUT(file_mas)
  DM_GET_OUTPUTS
    DM_OUTPUT(length_al)
...
void CCalcX::Calculate(bool dry_run) {
  /*
  length_al->set_value(0.);
  for(int li2 = 0; li2 < length->value(); li2++) {
    length_al->set_value(length_al->value() + (*vector_mask)[li2]);
  }
  */
  flaghky->set_value(0);
  ...
  if (include_unknown->value()) {
    if (!(file_mas->value()[0] == '-'  && file_mas->value()[1] == '1')) {
      for(int x = 0; x < int_total_nsam->value(); x++)
        (*sum_sam)[x] = (*sum_sam_mask)[x] /*- nmhits*/ ;
      length_al->set_value(length_mask->value()- nmhits->value()) ;
    }
...
//    for(int x = 0; x < int_total_nsam->value(); x++)
//      (*sum_sam)[x] = length->value() - nmhits->value();
//    length_al->set_value(length->value() - nmhits->value());
    
    for(int x=0;x<int_total_nsam->value();x++) {
      if (length->value() - nmhits->value() > 0)
        (*sum_sam)[x] = length->value() - nmhits->value();
      else
        (*sum_sam)[x] = 0;
    }
