...
CCalcArrayOpeX::CCalcArrayOpeX()
: ICalculation(KeyString::CALC_ARRAY_OPE_X,
               KeyString::CALC_ARRAY_OPE_X_BRIEF_DESC,
               KeyString::CALC_ARRAY_OPE_X_DESC,
  ...
  BEGIN_CALCULATION_INTERFACE_DEFINITION
    ...
    SET_OUTPUT_INFO(mdw_reg_lengths,                                            // Variable
                   UNDEFINED_STRING,                                            // Group
                   UNDEFINED_STRING,                                            // Short Name
                   UNDEFINED_STRING,                                            // Long Name
                   UNDEFINED_STRING,                                            // Description
...
void CCalcArrayOpeX::Prepare(void) {
  DM_GET_INPUTS
    DM_INPUT(ploidy)
  ...
  DM_GET_OUTPUTS
    DM_OUTPUT(mdw_reg_lengths)
  DM_END
}

void CCalcArrayOpeX::Calculate(bool dry_run) {
  mdw_reg_lengths->ReserveMemory(npops->value(), 0);

  for (int x=0; x<npops->value(); x++) {
    (*mdw_reg_lengths)[x] = static_cast<long int>(
            ((*vint_perpop_nsam)[x] * ((*vint_perpop_nsam)[x] - 1))/2);
  }
}

void CCalcArrayOpeX::Finalize(void) {
